<script setup lang="ts">
    ...
    function onInput() {
        output.value = applyTransformation(input.value as string);
    }
...
<template>
    <div class="flex flex-col h-full">
        <header class="flex px-8 py-3 shadow-sm z-10 bg-primary-50">
            <div left class="flex flex-1">
                <div class="hs-tooltip inline-block [--placement:bottom] mr-4 ">
                    <button class="hs-tooltip-toggle p-2 inline-flex rounded-md bg-primary-200 text-primary-800 align-middle hover:bg-primary-300 outline-none ring-0">
                        ...
                        <span class="hs-tooltip-content hs-tooltip-shown:opacity-100 hs-tooltip-shown:visible opacity-0 transition-opacity inline-block absolute invisible z-50 py-1 px-2 bg-primary-600 text-xs font-medium text-white rounded-md shadow-sm" role="tooltip">
                          Add
                        </span>
                    </button>
                </div>
        ...
        <main class="flex flex-1 bg-primary-200 relative">
            <div content class="xflex xflex-col xrelative w-full h-full">
                <Flow />
            </div>
            <div right class="flex flex-row absolute bottom-0 p-8 w-full">
                <div class="w-full rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-primary-600 focus-within:ring-offset-2 transition-all">
                    <textarea v-model="input" @input="onInput()" class="h-[16rem] flex-1 p-4 block w-full resize-none shadow-sm z-10 bg-primary-50 text-primary-800 text-sm outline-none border-none border-transparent outline-none focus:ring-0" autofocus placeholder="Input"></textarea>
                </div>
                <div class="flex-0 self-center m-2">
                ...
                <div class="w-full rounded-md overflow-hidden focus-within:ring-2 focus-within:ring-primary-600 focus-within:ring-offset-2 transition-all">
                    <textarea v-model="output" class="h-[16rem] flex-1 p-4 block w-full resize-none shadow-sm z-10 bg-primary-50 text-primary-800 text-sm outline-none border-none border-transparent outline-none focus:ring-0" placeholder="Output"></textarea>
                </div>
            </div>
        </main>

        <SettingsModal />
        <ContextMenu @optionSelected="optionSelected" />
    </div>
</template>
